...
Status: canonical short-form context for Mike, Codex and Lovable; linked contracts retain authority.
...
- App `e12482a3`; production `eb54d65b…`; containment baseline `c1cdc4a7e9ae4d16766125f7e56509affe6b79d4`.
- PX2B pre-merge checks passed: TypeScript, 88 tests, scoped lint and production build.
- Reminder job 6 is inactive; HTTP fulfilment fails closed with `503` unless enabled. No scheduled reminder was sent; requests receive an automatic transactional confirmation.
- Mike reported another subscriber on 12 August without a private-table audit: demand evidence, not fulfilment or value.
...
RENM will run a 120–150 day trusted UK race-intelligence experiment: rectify the catalogue, test the existing-backend Explorer and one authorised course-rich page, then add only evidenced modelling, feeds and distribution. Fixed signals determine whether to continue, narrow or stop major investment.

Prefer governing/licensing bodies, registration providers, timing/results providers and organisers. Strava/Garmin are optional runner-authorised enrichment; named results remain rights/privacy-gated. See [product reset](RENM-interactive-race-explorer-product-reset-2026-08-07.md).

Lovable validates; GitHub is source of truth and the build remains portable. Migrate only for measured constraints.

## Active non-negotiable boundaries

- Do not activate/test automated reminders against real subscribers or message historic subscribers.
...
- Do not ingest a feed without stable identity, update semantics, permitted use, provenance, corrections and reversible conflicts.
...
- Do not expose personal contacts, prospectability, raw provenance or moderation fields publicly or to Lovable.
...
- K1 knowledge control is complete; kernel, manifest and ledger govern and mirror the canon.
...
- L3A/L3A-R are live; accepted L3B region/homepage/county/city/distance packages are recorded; L3B-5B is paused.
- PX0 reset is active; app, schema, data, integration and deployment mutations remain separately gated.
...
- PX2B enrichment is accepted; residuals are bounded legacy/source-data work. See [PX2B](RENM-PX2B-source-enrichment-package-2026-08-12.md).
- PX2C `North Downs Run 2026` has a cleared attributed Plotaroute embed; custom GPX reuse still needs permission and edition confirmation. See [PX2C](RENM-PX2C-course-rich-event-page-package-2026-08-12.md).
- Soft-404 accepted: 43/43. GSC started 14 August: 551 pending, 0 failed.
- At `9558063`, event analytics changed to `Outbound Click` with conservative `destination_role`; a click remains only a hand-off.
- PX1 wayfinding is live at `5096035e…` (`9498494c…`): four manifests plus Saturn; 180 tests/build passed. No data/schema/provenance mutation. See [pilot](RENM-outbound-wayfinding-source-graph-pilot-2026-08-14.md).
- PX1 QL1 is accepted at `4cb0f228…`: licence-state wording and Sedgefield `Sedgefield Harriers` / `club` are live with all five exits. A stale-head publish was immediately rolled back; replacement deployment and audited reapply passed all five pilots without console errors. No schema, club entity or wider data change. See [QL1](RENM-event-page-information-depth-QL1-preview-2026-08-15.md).
- PX1 outbound-wayfinding pilot closeout is complete. No further pilot acceptance work blocks QL2; the temporary Sedgefield transition guard remains a separately verified later cleanup, not a data-work prerequisite.
...
1. PX0: maintain canon/baseline verification and prepare the England Athletics/partner brief.
2. PX1: begin QL2 read-only evidence and exact existing-schema previews, then continue QL3–QL5 only through separate gates; no bulk enrichment/organiser graph. See [gap map](RENM-event-page-information-depth-gap-map-2026-08-15.md).
3. PX2: retain SEO entrances and test the existing-backend Explorer plus attributed North Downs course; custom route tooling requires permission and edition confirmation.
4. PX3: add PostGIS, `pg_trgm`, broader schema or integrations only for demonstrated needs.
5. PX4 tracked data-led search/social/video distribution and return behaviour.
6. PX5 day-120–150 gate: continue, narrow or stop; assess platform fit without assuming full-stack migration.
...
- D57–D67: run the time-boxed experiment with authorised sources, field provenance, measurable distribution and a portable build; graduate platforms only from demonstrated need.
- D68: finish existing-schema rectification and bounded tests before PostGIS, `pg_trgm` or broader schema.
