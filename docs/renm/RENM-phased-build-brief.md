...
4. **Organiser relationship and ORL.** The next bounded package is a review of intake and reconciliation over the **existing** ORL plumbing — not a new organiser graph, an organiser portal, a broad schema build or a direct data rewrite. Preserve organisations, aliases, `organisation_platform_accounts`, `identity_evidence` with fingerprints, unique typed `organisation_event_links`, link-evidence joins, the append-only reviews/state machine, unresolved seed quarantine, the hash-locked intake queue/validator and the admin review surfaces. Route every available clue from the gap/event inventory into that plumbing; match against canonical organisations, aliases, platform accounts, tenant/platform identifiers and evidence fingerprints; preserve typed relationships (`organises`, `entry_platform_hosts`, `source_suggests`); retain unresolved candidates; and consolidate the evidence bucket for human review. Facebook, SI Entries, Eventrac, EntryCentral and similar multi-tenant evidence is kept as typed candidate evidence, never discarded for host ambiguity and never treated as the organiser because of its host. The host matcher is provisional intake assistance only and must not manufacture a deterministic organiser. The Organiser Gap page stays a read-only inventory/intake and prioritisation view; the public events row stays a projection never overwritten from a host match; broad legacy organiser backfill is out of scope without separate review. Aggregated, PII-free demand may prioritise review order only. Integration remains review/planned work pending separate approval.
