...
**Organiser and occurrence evidence doctrine.** The Organiser Review Layer (ORL) is the single canonical organiser identity, evidence and approval authority — and it is not an organiser-name approval table. It is the private evidence-resolution and review graph that consolidates the disparate threads validating a race occurrence and its relationships: canonical organisations; aliases, trading and source names; organiser and club sites; organiser-controlled Facebook and social endpoints; governing-body observations; registration and booking platforms such as SI Entries, Eventrac and EntryCentral; platform accounts and tenant identifiers; source clues; duplicate-row caveats; and multiple evidence observations supporting one unique typed relationship.

Consistent with the link and action doctrine, channel and role remain separate. A Facebook endpoint may support identity or official-information authority; an event-specific registration-platform record may support occurrence identity and the runner's action destination; neither becomes the organiser merely because of its host. The full endpoint, path and platform identifier, the specific claim, the evidence and the corroborating relationships all matter.

Facebook, SI Entries and other multi-tenant or platform evidence is never discarded because host-level matching is ambiguous. It is preserved as typed candidate evidence and routed to reconciliation; ambiguity creates candidate and review work, not a deterministic answer. Unknown remains preferable to an unsupported conclusion.

The Organiser Gap page is a read-only inventory, intake and prioritisation view over missing, incomplete or unresolved ORL evidence — never a parallel flat organiser system and never a direct host-to-organiser backfill mechanism. Integration routes gap and event clues into the existing ORL intake and reconciliation plumbing, matching against canonical organisations, aliases, platform accounts, tenant/platform identifiers and evidence fingerprints, preserving typed relationships (`organises`, `entry_platform_hosts`, `source_suggests`), retaining unresolved candidates and consolidating the evidence bucket for human review. Reviewed, claim-specific conclusions then support occurrence validation, official-information and next-action decisions.

The existing plumbing is preserved, not rebuilt or bypassed: organisations, aliases, `organisation_platform_accounts`, `identity_evidence` with fingerprints, unique typed `organisation_event_links`, link-evidence joins, the append-only reviews and state machine, unresolved seed quarantine, the hash-locked intake queue and validator, and the admin review surfaces.

The organiser-resolution host matcher is provisional intake assistance only. A host-only match cannot establish canonical identity or relationship; where a host maps to several clubs or is a multi-tenant, social or registration platform it must retain the clue for ORL candidate reconciliation rather than choose the first club. Broad direct legacy organiser backfill is not a substitute for ORL reconciliation and needs separate review and approval.

Per the Data and Lifecycle Contract, the public events row stays a projection and operational record: it is never overwritten directly from a host match, and applying reviewed ORL conclusions to public fields or gates is a separate audited, approved step. Demand signals such as search clicks and reminder requests may prioritise review order but are never identity or trust evidence, and organiser opportunity evidence is aggregated and excludes runner PII.
...
**Observed evidence, 18 September 2026 (volatile, not doctrine).** 36 stored reminder requests from 34 distinct addresses in the recent 90-day window, two addresses with more than one race — the agreed 25-request prototype trigger is met and repeat intent is measured separately. 1,181 future ACTIVE events with 141 usefully named organisers (12%), 108 `TBC` and 10 `Unknown` rows from a single 8 May import, and 575 unnamed England Athletics events of which 566 hold `organiser_url` evidence. ORL holds 8 organisations, 5 aliases, 9 platform accounts, 21 identity-evidence observations, 2 accepted organisation–event links and one linked future event — proven starter plumbing rather than broad inventory coverage, with intake and reconciliation not yet connected to the wider catalogue. September Lovable analytics showed a large direct/desktop anomaly against the earlier Google/mobile profile; exclude bots, monitoring and measurement changes before relying on it.
...
**Context recovery, 18 September 2026 (provenance record, not doctrine).** The prior Windows desktop OS, its Codex local/worktree workspace and the exact Codex conversation transcript were wiped and are not recovered. Uncommitted local-only code or plans from that machine remain unrecoverable unless they existed in another backup. The durable project baseline was substantially recovered from the connected GitHub repository and history, Lovable project source/history/knowledge, versioned canonical documents, migrations/tests/audits/acceptance records, and read-only production observations — including the operating kernel, Project Knowledge, the data and lifecycle contract, the phased build brief, the decision register through D68 (since extended through D75), reminder containment history, ORL architecture and evidence model, and audits and implementation/acceptance history. The 18 September checkpoint and D69–D75 extend that recovered baseline; the project continues from a reconstructed evidence-backed canon, not from memory. The recovered baseline is substantial but not identical to the lost transcript: where a claim exists only in recollection and not in durable evidence, label it as recollection or re-verify it. GitHub remains the source of truth; Lovable history is supporting implementation/context evidence; production observations are time-stamped evidence rather than doctrine.
