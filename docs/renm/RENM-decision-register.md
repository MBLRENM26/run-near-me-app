...
## Decisions recorded at the reminder-operations checkpoint — 18 September 2026

| ID | Decision | Consequence |
|---|---|---|
| D69 | 25 stored race-reminder requests remains the agreed prototype trigger, and the 18 September 2026 production observation of 36 requests from 34 distinct addresses meets it. | Proceed to scope a bounded runner-reminder prototype. Repeat intent (two addresses with more than one race) is measured separately as a secondary observation, not an additional veto or retrospective gate. These remain interest signals, not paid intent, delivered value, entries or revenue. |
| D70 | Build and prove the operational reminder flow before copy, pricing or wider promotion. | Deliver the manual Reminder Operations loop — request, confirmation, real-time admin notification, visible task, link/timing verification, schedule, manual preview/send, delivery/suppression/failure/retry, click and labelled self-reported outcome — and pass its acceptance before any commercial or promotional work. |
| D71 | Reminder fulfilment stays manual-first and fail-closed, and a failed send must remain retryable. | Job 6 remains inactive and sending requires `REMINDER_SENDING_ENABLED` to be exactly `true`. Marking `reminder_sent_at` after a failed enqueue is a defect to correct; failures stay visible and actionable. Automation follows only after manual acceptance evidence and separate approval, retaining kill switch, idempotency, capped retries and visible failures. |
| D72 | Email and social distribution share one verified-data, eligibility, scheduling, approval, status and audit infrastructure, with social following email. | Build email first on that shared infrastructure; initial social automation produces drafts for approval only, with no raw-feed-to-public automation. |
| D73 | Event-specific reminder consent remains purpose-limited, and organiser-facing evidence is aggregated without runner PII. | Race-specific transactional consent may not be reused for newsletters or marketing; broader alerts require separate explicit optional consent. No historic or bulk subscriber messaging; historic records need individual eligibility and purpose review. Organiser opportunity evidence discloses no runner identities. |
| D74 | The Organiser Gap page feeds the Organiser Review Layer rather than becoming a parallel organiser identity authority. | ORL remains the single canonical organiser identity, evidence and approval authority; the gap page stays read-only intake and proposal evidence. Ambiguous website-host-to-club evidence must produce no deterministic proposal. Implementation of the intake integration remains subject to the dedicated read-only review and separate approval. |
| D75 | ORL is the private evidence-resolution and review graph for occurrence and relationship validation, not an organiser-name approval table or a host-to-organiser labelling mechanism. | Consolidate all validating threads in ORL: canonical organisations; aliases/trading/source names; organiser and club sites; organiser-controlled Facebook/social endpoints; governing-body observations; registration/booking platforms such as SI Entries, Eventrac and EntryCentral; platform accounts and tenant identifiers; source clues; duplicate-row caveats; and multiple observations supporting one unique typed relationship. Keep channel and role separate — a Facebook endpoint may support identity or official-information authority and an event-specific registration record may support occurrence identity and the runner's action destination, but neither is the organiser because of its host; the full endpoint/path/platform identifier, claim, evidence and corroborating relationships govern. Never discard Facebook, SI Entries or other multi-tenant/platform evidence for host ambiguity: retain it as typed candidate evidence and route it to reconciliation, since unknown is preferable to an unsupported conclusion. Route gap/event clues into the existing plumbing (organisations, aliases, `organisation_platform_accounts`, `identity_evidence` with fingerprints, unique typed `organisation_event_links`, link-evidence joins, append-only reviews/state machine, unresolved seed quarantine, hash-locked intake queue/validator, admin review surfaces), preserving typed relationships including `organises`, `entry_platform_hosts` and `source_suggests`. The host matcher is provisional intake assistance only and must not choose the first club or manufacture a deterministic organiser; broad direct legacy organiser backfill is prohibited as a substitute for reconciliation without separate review. The public events row stays a projection never overwritten from a host match, and demand signals may prioritise review order only, never identity or trust, with no runner PII. Implementation remains separately gated: this documentation authorises no code, schema, data, integration, job, UI, deployment or bulk-backfill mutation. |
| D76 | The recovered durable canon is accepted as the operating baseline after loss of the original local Codex workspace/transcript. | The prior Windows desktop OS, its Codex local/worktree workspace and the exact conversation transcript were wiped and are not recovered; uncommitted local-only code or plans remain unrecoverable unless they existed in another backup, and no recovery claim implies otherwise. The durable baseline was substantially recovered from the GitHub repository and history, Lovable project source/history/knowledge, versioned canonical documents, migrations/tests/audits/acceptance records and read-only production observations, and the 18 September checkpoint and D69–D75 extend it. Continue from this reconstructed evidence-backed canon rather than restarting from memory. Preserve provenance and uncertainty: where a claim exists only in recollection and not in durable evidence, label it as recollection or re-verify it, and never reconstruct missing details as fact. GitHub remains source of truth; Lovable history is supporting implementation/context evidence; production observations are time-stamped evidence rather than doctrine. |
