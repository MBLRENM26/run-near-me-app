...
4. **Organiser relationship and ORL.** Route every available clue from the gap/event inventory into the existing ORL intake and reconciliation plumbing; match against canonical organisations, aliases, platform accounts, tenant/platform identifiers and evidence fingerprints; preserve typed relationships including `organises`, `entry_platform_hosts` and `source_suggests`; retain unresolved candidates; consolidate the evidence bucket for human review; and use reviewed, claim-specific conclusions to support occurrence validation, official-information and next-action decisions. The gap page stays a read-only inventory, intake and prioritisation view over missing, incomplete or unresolved ORL evidence — never a parallel flat organiser system or a direct host-to-organiser backfill mechanism. Demand signals (search clicks, reminder requests) may prioritise review order but are never identity or trust evidence and disclose no runner PII. This integration stays review/planned work until separately approved.
