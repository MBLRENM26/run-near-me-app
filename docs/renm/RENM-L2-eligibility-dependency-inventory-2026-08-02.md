...
5. **`duplicate_of` is almost universally ignored.** Only `getLiveStats` and `search_events_v1` filter it. **[OE]** 38 rows are `status='ACTIVE'` **and** `duplicate_of IS NOT NULL` — these are excluded from the headline count and from search, but are fully discoverable on landing pages and directly reachable. Example: `east-grinstead-10k-andy-ripley-memorial-2025` (`0333ad39-…`), discoverable = true, in sitemap = false (`duplicate-sibling`), counted = false.
