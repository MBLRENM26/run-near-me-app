CREATE OR REPLACE FUNCTION public.events_within_radius(p_lat double precision, p_lng double precision, p_radius_miles double precision, p_max_results integer DEFAULT 500)
 RETURNS TABLE(id uuid, name text, date_raw text, town text, county text, distance_type text, entry_fee text, url text, is_featured boolean, distance_miles double precision)
 LANGUAGE sql
 STABLE
