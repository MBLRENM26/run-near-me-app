...
import type { TaxonomyPageConfig } from "@/lib/taxonomy-pages";
...
interface Props {
  cfg: TaxonomyPageConfig;
  data: TaxonomyPageData;
}

export function TaxonomyLandingPage({ cfg, data }: Props) {
  ...
  useEffect(() => {
    trackRegionView({ region: "UK", distance: cfg.slug, total_events: total });
  }, [cfg.slug, total]);
          ...
          {filtered.length === 0 ? (
            <div className="text-center py-16 rounded-2xl border border-dashed border-border bg-muted/30">
              <p className="text-lg font-medium text-foreground">
                {month
                  ? `No ${cfg.nounPlural} in ${formatMonthLabelLong(month)} yet`
        ...
        {regionCounts.length > 0 && (
          <section className="mx-auto max-w-6xl px-4 pb-12">
            ...
            <p className="mt-2 text-muted-foreground">
              Browse {cfg.nounPlural} by UK region.
            </p>
            <div className="mt-6 grid gap-3 grid-cols-2 sm:grid-cols-3 lg:grid-cols-4">
              {REGIONS.map((region) => {
                const c =
                  regionCounts.find((rc) => rc.region === region.name)?.count ??
                  0;
                return (
                  <Link
                    key={region.slug}
...
export function buildTaxonomyHead(
  cfg: TaxonomyPageConfig,
  data: TaxonomyPageData | undefined,
) {
  ...
  const collectionLd = {
    ...
    description,
    url: canonical,
    about: { "@type": "Thing", name: cfg.nounPlural },
  };
