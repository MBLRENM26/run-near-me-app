...
/**
 * L3B-3 regression: county discovery must read from the accepted public
 * projection `events_public_v1`, never the `events` base table, and must not
 * re-apply the redundant status predicate.
 */
const source = readFileSync(
  new URL("./county.functions.ts", import.meta.url),
  "utf8",
);
...
describe("county discovery data access (L3B-3)", () => {
  ...
  it("selects only columns within the accepted public projection", () => {
    const selected = DISCOVERY_EVENT_COLUMNS.split(",").map((column) =>
      column.trim(),
    );
