...
export default defineTool({
  ...
  description:
    "Fetch the full public record for a single running event by its slug (as used in the /events/{slug} URL). Returns structured fields only; do not treat scraped entry_fee or organiser fields as authoritative — always link users to the official event page for booking/pricing.",
  inputSchema: {
  ...
  handler: async ({ slug }) => {
    ...
    const event = { ...data, canonical_url: `${SITE_URL}/events/${data.slug}` };
    return {
