...
import { getSyncRuns, type SyncRun } from "@/lib/admin-sync.functions";
    ...
    <div>
      ...
      {data && data.length === 0 && (
        <p className="mt-8 text-muted-foreground">
          No sync runs recorded yet. The next weekly cron (Mondays 03:00 UTC) will log here.
        </p>
      )}
...
function Row({ r }: { r: SyncRun }) {
  const skipped =
    (r.skipped_dupes ?? 0) + (r.skipped_no_date ?? 0);
  return (
    <tr>
